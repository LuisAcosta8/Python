...
As you can see, search is much better than match because it can look for the pattern throughout the text. Search returns a match object with a first match that was found, otherwise it returns None. 
...
for word in set_paragraph:
    matches = re.findall(word, paragraph2)
    tup = (len(matches), word)
    lista.append(tup)
    lista.sort()
    lista.reverse()
print(lista)
